		...
		try {
			...
			case 1: {
				if (kipper.load("save.txt"))
					...
					throw Exception("Файл не найден или повреждён\n");
				break; }
			case 2: {
				int type;
				cout << "\tВыбирите Корабль:\n"
					<< "\n\t1.Подводная лодка"
					<< "\n\t2.Катер"
					<< "\n\t3.Парусная лодка\n\t->";
				cin >> type;
				...
				default: throw Exception( "\tВыбрано недопустимое значение!\n");
				}
				break; }
			case 3: {
				if (kipper.size() == 0)
			...
			case 4: {
				...
				else
				{
					...
					{
						throw Exception ("\tВыбрано недопустимое значение!\n");
					}
				}
				break; }
			case 5: {
				if (kipper.save("save.txt"))
					...
					throw Exception( "\tПри сохранении произошла ошибка\n");
				break; }
			case 0: {
